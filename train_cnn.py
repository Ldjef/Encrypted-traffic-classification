...
@click.option('-t', '--task', default='traffic', help='classification task. Option: "app" or "traffic"', required=True)
...
def main(data_path, model_path, task, gpu):
    ...
    if task == 'app':
        ...
        cm=confusion_matrix(data_path + '/processed_test_data/',model,num_class=5)
        print(get_classification_report(cm,labels=ID_TO_TRAFFIC))
    else:
        exit('Not Support')
