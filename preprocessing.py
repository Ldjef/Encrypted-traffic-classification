...
def transform_pcap(path, output_path: Path = None, output_batch_size=10000):
    #mymask 如果已经处理过，有SUCCESS文件存在，则跳过
    ...
    for i, packet in enumerate(read_pcap(path)):
        ...
        if arr is not None:
            ...
            #mymask 有些标签为none，过滤掉
            if(traffic_label==None or app_label is None):
                count+=1
                continue
            #mymask 标签有两种，app_label,traffic_label,剩下的是特征
            row = {
                'app_label': app_label,
    ...
    with Path(str(output_path.absolute()) + '_SUCCESS').open('w') as f:
        f.write('')
    print('total no label count:',count)
    time.sleep(2)
    print(output_path, 'Done')


@click.command()
@click.option('-s', '--source', default='./VPN-PCAPS-01/',help='path to the directory containing raw pcap files', required=True)
